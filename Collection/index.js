...
var Collection = exports.default = /*#__PURE__*/function () {
   ...
   * subscribers.
   */;
  _proto.refreshCache = function (clauses) {
    var _this4 = this;
    return new Promise(function (resolve) {
      _this4._unsafeFetchRaw(new _Query.default(_this4, clauses), function (results) {
        var _results$value;
        var updateCacheOperations = [];
        var notifySubscribersOperations = [];
        null === (_results$value = results.value) || void 0 === _results$value ? void 0 : _results$value.map(function (rawRecord) {
          rawRecord = (0, _RawRecord.sanitizedRaw)(rawRecord, _this4.schema);
          var record = _this4._cache.recordInsantiator(rawRecord);
          _this4._cache.delete(record);
          updateCacheOperations.push({
            record: record,
            type: 'created'
          });
          if ('created' === record._raw._status || 'updated' === record._raw._status || 'destroyed' === record._raw._status) {
            notifySubscribersOperations.push({
              record: record,
              type: record._raw._status
            });
          }
        });
        _this4._applyChangesToCache(updateCacheOperations);
        _this4._notify(notifySubscribersOperations);
